...
def getSushiStats(credentialFile):
    ## Lê um dos arquivos na pasta que deve ter as informações conforme a ordem abaixo:
        with open(credentialFile) as csvfile:
            ...
            print('Lendo CSV' + '' + credentialFile)
            ...
            for row in readCSV:
                ...
                pycounter.report5.CounterReport.write_tsv(objectCounterReport,pathToTSVfile)
                print("\n---------------\n")
...
if siglaDaEditora == "TODOS":
    ...
    for file in dirs:
        ## print para debug
        print(file)
        fileName = pathToCredentials + file
        getSushiStats(fileName)
...
## Para cá vem o programa quando o valor da variável editora não é TODOS
    ...
    fileName = pathToCredentials + siglaDaEditora + '.csv'
    getSushiStats(fileName)