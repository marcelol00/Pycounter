...
class CounterJournal(CounterEresource):
    ...
    def __init__(
        ...
        super(CounterJournal, self).__init__(
            cust_ID, inst_name, uri, period, metric, month_data, title, platform, publisher
        )
...
#Almeida Início
        ...
        if cust_ID is not None:
            ...
            self.cust_ID = CounterReport().customer

        if inst_name is not None:
            self.inst_name = inst_name
        else:
            self.inst_name = CounterReport.customer
...
#Almeida Fim
    ...
    def as_generic(self):
        ...
        """
            #Almeida - Comentei para que não adicionasse zeros às colunas "html e pdf_total" que eu removi do cabeçalho. 
