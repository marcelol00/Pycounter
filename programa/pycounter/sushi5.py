...
def _get_identifiers(item):
    ...
    identifiers = {"eissn": "", "issn": "", "doi": "", "prop_id": "", "isbn": "", "institution_Name": "", "customer_ID": "", "uri": ""}
    for identifier in item["Item_ID"]:
        ...
        elif identifier["Type"] == "Institution_Name":
            identifiers["institution_Name"] = identifier["Value"]
        elif identifier["Type"] == "Customer_ID":
            identifiers["customer_ID"] = identifier["Value"]
        elif identifier["Type"] == "uri":
            identifiers["uri"] = identifier["Value"]
...
def raw_to_full(raw_report):
    ...
    reportType = header["Report_ID"]
    report = pycounter.report5.CounterReport(
        period=period,
    ...
    for item in raw_report["Report_Items"]:
        ...
        if report.report_type == "TR_J1":
            for metric, data in metrics_data.items():
                report.pubs.append(
                    pycounter.report5.CounterJournal(
        ...
        elif report.report_type == "Silverchair:CR_TR_J1":
            for metric, data in metrics_data.items():    
                report.pubs.append(
                    pycounter.report5.CounterJournal(
                        inst_name=item.get("Institution_Name", ""),
